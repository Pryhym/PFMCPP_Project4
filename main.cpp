...
struct DoubleType
{
    ...
    DoubleType& apply(void(*ptr)(double&));
...
DoubleType& DoubleType::apply(void(*ptr)(double&))
{
    if (ptr)
    {
        ptr(*value);
...
IntType& IntType::apply(void(*ptr)(int&))
{
    if(ptr)
    {
        ptr(*value);
...
void myDoubleFreeFunct(double& dbl)
{
    dbl += 6.0;
}
...
void part6()
{
    ...
    ft3.apply( [&](float&)->FloatType&{ft3+=7.0f; return ft3;} );
    ...
    dt3.apply( [&](double&)->DoubleType&{dt3+=6.0; return dt3;} );
    ...
    it3.apply( [&](int&)->IntType&{it3+=5; return it3;} );
